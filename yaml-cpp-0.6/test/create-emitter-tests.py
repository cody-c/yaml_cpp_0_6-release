...
def gen_templates():
    yield [[doc_start(), doc_start(True)],
           [scalar('foo'), scalar('foo\n'), scalar(
               'foo', 'tag'), scalar('foo', '', 'anchor', 1)],
           [doc_end(), doc_end(True)]]
    yield [[doc_start(), doc_start(True)],
           [seq_start()],
           [[], [scalar('foo')], [scalar('foo', 'tag')], [scalar('foo', '', 'anchor', 1)], [scalar('foo', 'tag', 'anchor', 1)], [
    ...
    yield [[doc_start(), doc_start(True)],
           ...
           [[], [scalar('foo'), scalar('bar')], [scalar(
               'foo', 'tag', 'anchor', 1), scalar('bar', 'tag', 'other', 2)]],
           [map_end()],
           [doc_end(), doc_end(True)]]
...
def gen_tests():
    for events in gen_events():
        name = 'test' + hashlib.sha1(''.join(yaml.dump(event)
                                             for event in events)).hexdigest()[:20]
...
def create_emitter_tests(out):
    ...
    with Scope(out, 'namespace YAML_0_6', 0) as _:
